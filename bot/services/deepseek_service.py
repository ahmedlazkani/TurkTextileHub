...
async def generate_channel_post(
    ...
    system_prompt = (
        ...
        "0. OUTPUT FORMAT: Use ONLY plain text with emojis. NEVER use Markdown (no **bold**, no *italic*, no __underline__, no `code`).\n"
        "   The post will be sent via Telegram's HTML parse mode — do NOT add any markup.\n"
        "   Labels like 'Beden', 'Kumaş', etc. should appear as plain text followed by a single colon, e.g.: 📌 Beden: M, L, XL\n"
        "   NEVER write 'Beden:' twice or add extra colons.\n\n"
        "CRITICAL DATA RULES:\n"
        ...
        "E. NOTES: If notes field is empty or '(none)', OMIT the notes line entirely — do not show it.\n\n"
        "STYLE RULES:\n"
        "1. Use relevant emojis (✅ 🔥 📌 💰 etc.) but keep it professional — max 2-3 per section.\n"
        "2. Format each language section like this (plain text, no markdown):\n"
        "   ────────────────────────\n"
        "   [Flag emoji] [Language Name]\n"
        "   [Product emoji] [Product Name]\n"
        "   [Description — 1-2 lines max]\n"
        "   ────────────────────────\n"
        "   📌 [Size label]: [sizes — ONLY from attributes, OMIT ENTIRELY if no size in attributes]\n"
        ...
        "   Arabic: المقاسات / القماش / السعر / كود\n"
        "   Turkish: Beden / Kumaş / Fiyat / Kod\n"
        "   English: Size / Fabric / Price / Code\n"
        "5. Return ONLY the post text — no explanations, no markdown code fences, no extra blank lines at start/end."
    )
